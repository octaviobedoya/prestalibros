...
exports.addBook = function(req, res) {
  ...
  if (typeof bookReq.Documento == 'undefined')
    res.json({ status: "error", message: "Documento required." });
  if (typeof bookReq.NombreCompleto == 'undefined')
    res.json({ status: "error", message: "NombreCompleto required." });
  if (typeof bookReq.Telefóno == 'undefined')
    res.json({ status: "error", message: "Telefóno required." });
  if (typeof bookReq.Email == 'undefined')
    res.json({ status: "error", message: "Email required." });
  if (typeof bookReq.TituloLibro == 'undefined')
    res.json({ status: "error", message: "TituloLibro required." });
  if (typeof bookReq.CódigoISBN == 'undefined')
    res.json({ status: "error", message: "CódigoISBN required." });
  if (typeof bookReq.Cantidad == 'undefined')
    res.json({ status: "error", message: "Cantidad required." });
  if (typeof bookReq.FechaEntrega == 'undefined')
    res.json({ status: "error", message: "FechaEntrega required." });



  // Prepare the query with the user response
  var book = bookModel({
    Documento: bookReq.Documento,
    NombreCompleto: bookReq.NombreCompleto,
    Telefóno: bookReq.Telefóno,
    Email: bookReq.Email,
    TituloLibro: bookReq.TituloLibro,
    CódigoISBN: bookReq.CódigoISBN,
    Cantidad: bookReq.Cantidad,
    FechaEntrega: bookReq.FechaEntrega
  });
